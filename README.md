...
一个功能强大的 Chrome 扩展，用于录制网页中的视频内容。支持多标签页选择、实时计时、异常处理、**分段录制**等功能。
...
- **🆕 分段录制**：支持长时间录制，每30分钟自动保存一段，适合录课需求
...
4. 录制开始后，页面右上角会出现悬浮窗
5. **分段录制**：每30分钟自动保存一段，录制结束后自动合并
6. 点击悬浮窗的"停止"按钮或关闭页面即可保存视频
...
- **无缝衔接**：自动开始下一段录制，无中断
- **智能合并**：录制结束后自动合并所有片段
- **实时监控**：悬浮窗显示当前片段、已保存片段数、总时长
- **错误恢复**：录制异常自动重试，确保不丢失内容

### 📁 文件管理

- **分段文件**：`课程名_第1段_时间戳.webm`
- **合并文件**：`课程名_完整录制_时间戳.webm`
- **自动清理**：合并后自动删除临时分段文件
...
const recorder = new MediaRecorder(stream, {
  ...
  videoBitsPerSecond: 3_000_000         // 视频码率 3Mbps（分段录制优化）
});
```
...
- **分段时长**：修改 `segmentDuration` 参数（默认30分钟）
